...
<div style="background:rgb(0 0 0 / 45%);border-radius:10px;padding:15px;">
    ...
    <button type="submit" class="btn btn-primary" @click="addDevice">提交</button>
...
export default {
    ...
    methods:{
        ...
        addDevice(){
            ...
            let newDevice = {deviceId:0,name:this.name,status:null,startTime:'',closeTime:'',token:access_token.token};
            let apiUrl = "http://localhost:8080/api/deviceInfo/add";  //判断token的接口
            let _this = this;
            axios.post(apiUrl,newDevice).then(
            function(response){
